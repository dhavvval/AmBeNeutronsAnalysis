...
efficiency_data = glob.glob(os.path.join(path, 'TriggerSummary/AmBeTriggerSummaryportLowerValue.csv'))
...
plt.title("AmBe neutron efficiency from AmBe 2.0 for Lower Value (PE < 150, CB < 0.4)")
plt.xlabel("Ports")
...
plt.tight_layout()
plt.savefig("OutputPlots/AmBeNeutronEfficiency_AmBe2.0wLowerValue.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.title("Residual Efficiency of AmBe 2.0 compare to AmBe 1.0 with Lower Value (PE < 150, CB < 0.4)")
plt.xlabel("Ports")
plt.ylabel("Y Position (cm)")
plt.xticks(rotation=45)
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig("OutputPlots/ResidualEfficiency_AmBe2.0wLowerValue.png", dpi=300, bbox_inches='tight')
plt.show()
