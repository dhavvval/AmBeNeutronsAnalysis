...
# Port information mapping - kept in sync with plots/heatmap.py PORT_INFO
# and plots/basic.py AmBeNeutronAnalyzer.port_info.
PORT_INFO = {
    ...
    (0, -50, 102): 'Port 3', (0, -60, 102): 'Port 3', (0, -100, 102): 'Port 3',
...
COUNT_COLUMNS = [
    'total_events', 'cosmic_events', 'single_neutron_candidates',
    'multiple_neutron_candidates', 'unique_neutron_triggers',
    'accepted_waveforms', 'rejected_waveforms',
]


def generate_stats(waveform_csv, trigger_csv, campaign_label, output_pdf):
    """
    Build the per-port "event selection flow" stacked-bar summary (accepted
    waveforms / AmBe triggers / cosmic / single & multiple neutron candidates,
    all as a % of total_waveforms) for one campaign.

    waveform_csv: path to AmBeWaveformResults_<campaign>.csv
                  (columns: sourceX, sourceY, sourceZ, run, accepted_waveforms,
                  rejected_waveforms, total_waveforms)
    trigger_csv:  path to AmBeTriggerSummary_<campaign>.csv
                  (columns: x_pos, y_pos, z_pos, total_events, cosmic_events,
                  ambe_triggers, single_neutron_candidates,
                  multiple_neutron_candidates, unique_neutron_triggers)
    """
    Wdf = pd.read_csv(waveform_csv).rename(columns={
        'sourceX': 'x_pos', 'sourceY': 'y_pos', 'sourceZ': 'z_pos',
    })
    Tdf = pd.read_csv(trigger_csv)

    # Historical position fix-up, same as heatmap.py / basic.py
    for df in (Wdf, Tdf):
        fix = (df['x_pos'] == 0) & (df['y_pos'] == -105.5) & (df['z_pos'] == 102)
        if fix.any():
            df.loc[fix, 'y_pos'] = -100

    WT_df = pd.merge(Wdf, Tdf, on=['x_pos', 'y_pos', 'z_pos'], how='outer')

    for pos in PORT_INFO.keys():
        missing = not ((WT_df["x_pos"] == pos[0]) &
                       (WT_df["y_pos"] == pos[1]) &
                       (WT_df["z_pos"] == pos[2])).any()
        if missing:
            row = {"x_pos": pos[0], "y_pos": pos[1], "z_pos": pos[2],
                   "ambe_triggers": 1, "total_waveforms": 1}
            row.update({col: 0 for col in COUNT_COLUMNS})
            WT_df = pd.concat([WT_df, pd.DataFrame([row])], ignore_index=True)
    ...
    WT_df["% Accepted waveforms"] = round((WT_df["accepted_waveforms"] / WT_df["total_waveforms"]) * 100, 2)
    ...
    WT_stats = WT_df[['port', "position", "total_waveforms", "accepted_waveforms", "ambe_triggers", "% Accepted waveforms",
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        for port in ports:
            ...
            ax.set_title(f'Event Selection Flow for {campaign_label} - {port}', fontsize=14)
...
def run(ctx, argv=None):
    """Not yet ported to RunContext API. Call generate_stats() directly."""
    raise NotImplementedError(
        f"This module ({__name__}) has not been ported to the ambe CLI yet. "
        "Call generate_stats(waveform_csv, trigger_csv, campaign_label, output_pdf) directly."
    )
